...
let score = 0;
console.log(
    chalk.bgYellowBright('Welcome to Guess a Number Game')
);
...
async function guessRandomNumber(){
    ...
    if(numberinput == randomNumber){
        score += 10;
        console.log(
           chalk.greenBright('Congrats you guess the right number.')
        );
        ...
        console.log(
            chalk.red('Oppss number not matched.')
        );
    }
    ...
    if(!again){
        console.log(
            chalk.green(chalk.bold(`Your Score is ${score}`))
        )
    }
}
